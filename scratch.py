...
if __name__ == "__main__":
    ...
    ssim.object_to_source("Io", species=Species('Na', n_th=0, n_sp=80,
                             mass_per_sec=10**4.8, model_smyth_v_b=0.95*1000,
                             model_smyth_v_M=15.24*1000, lifetime=4*60, beta=0)
                          )
    #ssim.object_to_source("Europa", species=Species('H2', n_th=0, n_sp=80, mass_per_sec=10 ** 4.8, model_smyth_v_b=0.95 * 1000,
    #                                      model_smyth_v_M=15.24 * 1000, lifetime=4 * 60, beta=0)
    #                      )
    ...
    sa = SerpensAnalyzer(save_output=False, reference_system="Io")
    sa.plot_planar(timestep=4, d=2, scatter=True, triplot=True, show=True, trialpha=1,
                   lim=10, celest_colors=['orange', 'sandybrown', 'red', 'gainsboro', 'tan', 'grey'],
                   colormap=[cmap1, cmap2], show_source=True, single_plot=True)

    #sa.plot_lineofsight(timestep=2, show=True, show_primary=True, show_moon=True, lim=20,
